...
export class ChatsService {
  ...
  private async hydrate(rows: MessageRow[]): Promise<ChatMessageDTO[]> {
    ...
    const [parents, reactions, senders] = await Promise.all([
      parentIds.length
      ...
      senderIds.length
        ? this.prisma.user.findMany({
            where: { id: { in: senderIds.map((id) => BigInt(id)) } },
            select: { id: true, avatarUrl: true },
          })
        : Promise.resolve([] as { id: bigint; avatarUrl: string | null }[]),
    ]);

    const parentById = new Map(parents.map((p) => [p.id, p]));
    const avatarBySenderId = new Map<string, string | undefined>(
      senders.map((sender): [string, string | undefined] => [
        sender.id.toString(),
    ...
    return rows.map((r) =>
      this.rowToDto(
        ...
        r.senderId ? avatarBySenderId.get(r.senderId) : undefined,
      ),
    );
  }
